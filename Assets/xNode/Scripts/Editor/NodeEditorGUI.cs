...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        private float cachedTopPadding = 22f; 
        ...
        private float topPadding
        {
            get
            {
                ...
                {
                    cachedTopPadding = GUIUtility.GUIToScreenPoint(Vector2.zero).y - this.position.y;
                }

                return cachedTopPadding;
            }
        }
        ...
        public static Func<Event, bool> onProcessModalEvents;
        

        // Inside NodeEditorGUI.cs
        protected virtual void OnGUI()
        {
            if (onProcessModalEvents != null && onProcessModalEvents(Event.current)) {
                return; 
            }
            ...
            {
                // Clear context so the Inspector doesn't pick up xNode logic
                GUI.matrix = m;
        ...
        public static void BeginZoomed(Rect rect, float zoom, float topPadding) {
            GUI.EndClip(); // Escapes the default window clip

            // Apply scale
            GUIUtility.ScaleAroundPivot(Vector2.one / zoom, rect.size * 0.5f);
            
            // Calculate centering offset
            Vector2 clipOffset = new Vector2(
                -((rect.width * zoom) - rect.width) * 0.5f,
                -(((rect.height * zoom) - rect.height) * 0.5f) + (topPadding * zoom)
            );

            // Shift the matrix instead of the clip to fix the Unity 6 click bug
            GUI.matrix = GUI.matrix * Matrix4x4.Translate(clipOffset);

            // Re-push a stable clip starting at 0,0
            GUI.BeginClip(new Rect(0, 0, rect.width * zoom, rect.height * zoom));
        }

        public static void EndZoomed(Rect rect, float zoom, float topPadding) {
            // Restore the matrix to OS Window space (0,0) and shift below the header
            GUI.matrix = Matrix4x4.Translate(new Vector3(0, topPadding, 0));
        }
